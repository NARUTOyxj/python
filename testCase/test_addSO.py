...
class addSO(unittest.TestCase):
	"""docstring for TestLogin"""

	def setUp(self, api):
		readData = readConfig.ReadConfig()
		self.base_url = readData.get_section('HTTP','shipurl') + 'api/so'
		print(self.base_url)
	...
	def test_login(self):
		...
		with open(path, newline='') as f:
			...
			for row in reader:
				self.data = {base_args + 'SANumber':row['SANumber'],'departureWarehouseId':row['departureWarehouseId'],'destinationWarehouseId':['destinationWarehouseId'],'stockProductAttr':row['stockProductAttr'],'warrantyStatus':row['warrantyStatus']}
